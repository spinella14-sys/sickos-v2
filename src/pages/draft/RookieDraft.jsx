...
const AUTODRAFT_COUNTDOWN = 10  // seconds before auto-pick fires — matches the server-side autopick_fire_at window
...
export default function RookieDraft({ currentTeam, isCommissioner }) {
  ...
  // ── Load this team's saved "auto pick for me" setting ─────────────────────
  useEffect(() => {
    if (!team) return
    fetch(`${API}/draft/autopick-setting/${team}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d) setAutodraftOn(!!d.enabled) })
      .catch(() => {})
  }, [team])

  // Toggling persists to the backend so this survives closing the tab —
  // the server-side 10s fast-path in check-clocks reads this same setting.
  const toggleAutodraft = useCallback(() => {
    setAutodraftOn(prev => {
      const next = !prev
      fetch(`${API}/draft/autopick-setting`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ team_abbrev: team, enabled: next }),
      }).catch(() => {})
      return next
    })
  }, [team])
  ...
  // Fast poll of check-clocks every 3s while active -- this is what actually
  // makes the server-side clock/autopick fire (both the 10s opt-in fast-path
  // and the full pick clock). Any manager's open tab drives this, not just
  // the person on the clock -- so a pick still resolves even if their own
  // browser is closed.
  const checkClocksRef = useRef(null)
  useEffect(() => {
    if (checkClocksRef.current) clearInterval(checkClocksRef.current)
    if (draftState?.status === 'active' && team) {
      const poll = () => {
        fetch(`${API}/draft/cron/check-clocks`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ team_abbrev: team }),
        }).then(r => r.ok ? r.json() : null)
          .then(d => { if (d?.processed) fetchAll() })
          .catch(() => {})
      }
      checkClocksRef.current = setInterval(poll, 3000)
    }
    return () => { if (checkClocksRef.current) clearInterval(checkClocksRef.current) }
  }, [draftState?.status, team, fetchAll])
        ...
        <div style={{
          ...
          {autodraftOn && autoCountdown !== null && (
            <button className="draft-btn draft-btn--locked" style={{ fontSize:11, padding:'4px 10px' }}
              onClick={() => { clearTimeout(autoPickRef.current); clearInterval(autoClockRef.current); setAutoCountdown(null); toggleAutodraft() }}>
              CANCEL
            </button>
          )}
